...
# ===== 常量 =====
PERF_CLIENT_ID = "95727120-1781662325563@advertising.performance.ozon.ru"
PERF_CLIENT_SECRET = "TrOFrxJ5bic3GIFDa9H8PRLSzRggClV1CYbv3rAEVWb_4iySWlBzplMmh9I1yhLaypFkJq_q3yHeUolwCA"
PERF_BASE = "https://api-performance.ozon.ru"
SELLER_CLIENT_ID = str(config.OZON_STORE_KEYS['store8']['client_id'])
SELLER_API_KEY = config.OZON_STORE_KEYS['store8']['api_key']
...
def main():
    ...
    try:
        ...
        # 6. 保存到数据库
        sorted_skus = sorted(sku_orders.items(), key=lambda x: x[1]['orders'], reverse=True)
        ...
        store6_db.init_db('store8')
        store6_db.save_summary('store8', yesterday, analytics_units, analytics_money,
        ...
        store6_db.save_sku_list('store8', yesterday, sorted_skus, campaign_skus, ad_by_campaign, camp_sku_count, sku_ad_expenses)
        store6_db.save_campaigns('store8', yesterday, ad_stats, campaign_names, camp_sku_count)
        log("数据库已保存 ✅")
        ...
        try:
            tk = get_tenant_token()
            send_text(tk, OZON_GROUP_ID, f"❌ 店铺7日报执行失败: {str(e)[:200]}", 'chat_id')
        except:
            pass
        sys.exit(1)
